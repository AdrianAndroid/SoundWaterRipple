package com.flannery.soundwaterripple
...
class MainActivity : AppCompatActivity(), Runnable {
    ...
    private val handler: Handler = object : Handler(Looper.getMainLooper()) {
        override fun handleMessage(msg: Message) {
            ...
            val ratio = mMediaRecorder!!.maxAmplitude.toDouble() / 100 //mMediaRecorder.getMaxAmplitude()获取在前一次调用此方法之后录音中出现的最大振幅。
            var db = 0.0 // 分贝
...
//        voiceLineView.startWave();
        ...
        val file = File(Environment.getExternalStorageDirectory().path, "hello.log")
        if (!file.exists()) {
            try {
                file.createNewFile()
            } catch (e: IOException) {
